      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Panel titulo="Estado" className="lg:col-span-1">
          <div className="space-y-4 px-4 py-4">
            {/* gauge horómetro: elemento firma del sistema, versión grande */}
            <BarraHorometro tamano="grande" actual={actual} umbral={umbral} />

            <dl className="space-y-1 border-t border-line pt-3 text-sm">
              <div className="flex justify-between gap-4">
                <dt className="text-muted">{faltan >= 0 ? 'Faltan' : 'Operó de más'}</dt>
                <dd className={`font-mono ${faltan < 0 ? 'text-bloqueo' : ''}`}>
              ...
              <tbody>
                {equipo.maintenances.map((m) => (
                  <tr key={m.id}>
                    ...
                    <td
                      className={`${tabla.num} ${Number(m.overdueHours) > 0 ? 'text-aviso' : 'text-muted'}`}
                    >
