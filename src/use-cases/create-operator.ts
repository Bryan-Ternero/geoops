...
export async function createOperator(input: CreateOperatorInput) {
  ...
  if (code.length > 50 || fullName.length > 255 || document.length > 50) {
    throw new ServiceError({
      code: 'INPUT_TOO_LONG',
      message: 'Los datos del operador exceden la longitud máxima permitida (código: 50, nombre: 255, documento: 50 caracteres).',
      status: 400,
    });
  }
