...
import { formatIsoDate } from '../use-cases/dates';
...
export function MargenVsConsumo({ filas }: { filas: FilaMargen[] }) {
  ...
  return (
    <div className="px-4 py-4">
          ...
          return (
            <div key={f.code} className="grid grid-cols-[3.5rem_1fr_auto] items-center gap-3">
              ...
              <span className="relative block h-6 bg-canvas">
                ...
                <span
                  className="absolute inset-y-0 left-0 bg-ok/10"
                  style={{ width: pct(f.margen) }}
                />
                <span
                  className="absolute top-1/2 left-0 h-2.5 -translate-y-1/2 bg-ok"
                  style={{ width: pct(cubierto) }}
                />
                {exceso > 0 && (
                  <span
                    className="absolute top-1/2 h-2.5 -translate-y-1/2 bg-bloqueo"
              ...
              <span className="text-right font-mono text-xs whitespace-nowrap">
                <span className={f.cruza ? 'text-bloqueo' : ''}>
      ...
      <Leyenda
        items={[
          { color: 'bg-ok', label: 'Horas programadas dentro del margen' },
          { color: 'bg-bloqueo', label: 'Horas que exceden el umbral' },
          { color: 'bg-ok/10 border border-line', label: 'Margen disponible' },
...
export function DisponibilidadPorDia({ dias }: { dias: DiaFlota[] }) {
  ...
  return (
    <div className="px-4 py-4">
      ...
      >
        ...
        {dias.map((d, i) => {
          ...
          return (
            <g key={d.fecha}>
              {d.detenidos > 0 && (
                <rect x={x} y={y(total)} width={barra} height={altoDet} className="fill-bloqueo/20" />
              )}
              <rect
                x={x}
                y={y(d.disponibles)}
      ...
      <Leyenda
        items={[
          { color: 'bg-ok', label: 'Disponibles' },
          { color: 'bg-bloqueo/20', label: 'Detenidos por umbral, taller o baja' },
...
export function HistorialHorometro({
  ...
  return (
    <div className="px-4 py-4">
      ...
      >
        <polygon points={area} className="fill-ok/10" />
        <polyline
          points={linea}
          fill="none"
          className="stroke-ok"
        ...
        {umbral <= techo && (
          <>
            ...
            <text
              ...
              y={y(umbral) - 5}
              textAnchor="end"
              className="fill-bloqueo text-[10px]"
            >
      ...
      <Leyenda
        items={[
          { color: 'bg-ok', label: 'Horómetro acumulado' },
          { color: 'bg-taller', label: `Mantenimiento (${servicios.length})` },
          { color: 'bg-bloqueo', label: 'Umbral de bloqueo' },
        ]}
      />
    </div>
  );
